...
export default function ResumeTab() {
  ...
  const education: TimelineItem[] = [
    {
      ...
      date: "2022 – 2026",
      description: "Relevant Coursework: Advanced Networking, Database Systems, Object-Oriented Programming, Operating Systems, Data Structures, Algorithms, and Machine Learning.",
    },
    {
      title: "Data Structure, Algorithms & Software Dev",
      subtitle: "Africa to Silicon Valley (A2SV)",
      date: "2024 – 2025",
      description: "A2SV is an academy and incubator that empowers high-potential African students with rigorous software engineering training, preparing them for top tech companies.",
      bulletPoints: [
  ...
  return (
    <div id="resume-panel" role="tabpanel" className="flex flex-col gap-10 animate-fadeIn">
      {/* Page Title */}
            ...
            {experiences.map((exp, idx) => (
              <div key={idx} className="relative flex flex-col gap-2">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 text-xs">
                  <span className="font-semibold text-mint-light">{exp.subtitle}</span>
                  <span className="text-text-muted font-medium">{exp.date}</span>
                </div>
                {exp.description && (
          ...
          <div className="relative pl-6 border-l border-mint-primary/10 flex flex-col gap-8">
            {education.map((edu, idx) => (
              <div key={idx} className="relative flex flex-col gap-2">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 text-xs">
                  <span className="font-semibold text-mint-light">{edu.subtitle}</span>
                  <span className="text-text-muted font-medium">{edu.date}</span>
                </div>
                <p className="text-xs md:text-sm text-text-body leading-relaxed mt-1">
                  {edu.description}
                </p>
      ...
      {/* Skills Section */}
      <section className="flex flex-col gap-6">
        <h2 className="text-lg md:text-xl font-bold font-heading text-text-heading">My Skills</h2>
          ...
          {skills.map((skill, idx) => (
            <div key={idx} className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between text-xs font-semibold">
                ...
                <span className="text-mint-light">{skill.percentage}%</span>
              </div>
